...
public class StraightCablingHandsOn extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_straight_cabling_hands_on);
        ...
        btnTest.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(final View v) {
                ...
                testingPeriod.scheduleAtFixedRate(new TimerTask() {
                    @Override
                    public void run() {
                        StraightCablingHandsOn.super.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                ...
                                switch (testState) {
                                    ...
                                    case 19:
                                        if (
                                            /* accurate Cabling */
                                                accurateCabling(cable[0][0], cable[1][0], "WO") &&
                                                        accurateCabling(cable[0][1], cable[1][1], "O") &&
                                                        accurateCabling(cable[0][2], cable[1][2], "WG") &&
                                                        accurateCabling(cable[0][3], cable[1][3], "Blu") &&
                                                        accurateCabling(cable[0][4], cable[1][4], "WBlu") &&
                                                        accurateCabling(cable[0][5], cable[1][5], "G") &&
                                                        accurateCabling(cable[0][6], cable[1][6], "WBrn") &&
                                                        accurateCabling(cable[0][7], cable[1][7], "Brn")
                                                ) {
                                            Intent i = new Intent(StraightCablingHandsOn.this, AccurateCabling.class);
                                            startActivity(i);
                                            finish();
                                        } else if (
                                            /* precise Cabling */
                                                preciseCabling(cable[0][0], cable[1][0]) &&
                                                        preciseCabling(cable[0][1], cable[1][1]) &&
                                                        preciseCabling(cable[0][2], cable[1][2]) &&
                                                        preciseCabling(cable[0][3], cable[1][3]) &&
                                                        preciseCabling(cable[0][4], cable[1][4]) &&
                                                        preciseCabling(cable[0][5], cable[1][5]) &&
                                                        preciseCabling(cable[0][6], cable[1][6]) &&
                                                        preciseCabling(cable[0][7], cable[1][7])
                                                ) {
                                            Intent i = new Intent(StraightCablingHandsOn.this, PreciseCabling.class);
                                            startActivity(i);
                                            finish();
                                        } else {
                                            Intent i = new Intent(StraightCablingHandsOn.this, WrongCabling.class);
                                            startActivity(i);
                                            finish();
                                        }
                                        break;
    ...
    private void findSplitCables(CableTester[] RJ45A, CableTester[] RJ45B, String cableColor, Timer testingPeriod) {
        if (CableTester.checkDuplicateCables(RJ45A, RJ45B, cableColor)){
            Intent i = new Intent(StraightCablingHandsOn.this, SplittingCables.class);
            startActivity(i);
            testingPeriod.purge();
